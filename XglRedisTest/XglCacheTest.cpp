...
{
	std::string jjj = "okk";
	CCacheUtils::log0(LOG_DEBUG, "hello %s", jjj.c_str());
	std::shared_ptr<CCacheUtils> redis = CCacheUtils::createInstance("Z");
	...
	//subs test
	int getCnt = 0;
	std::shared_ptr<CCacheUtils> redis1 = CCacheUtils::createInstance("Z");
	bRlt = redis1->connect(REDISIP, REDISPORT, true);
	if (!bRlt)
	{
		logInfo << "connect failed... ip = " << REDISIP << ", port = " << REDISPORT;
	...
	redis1->subs("*", [&getCnt](const std::string & strKey, const std::string & strValue) {
		...
		logInfo << "subs callback job done... key = " << strKey.c_str();
		setLog(LOG_DEBUG, logInfo);
	});
